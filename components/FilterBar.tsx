...
export function FilterBar({ events, tags, hosts }: FilterBarProps) {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.55fr)_minmax(19rem,0.9fr)] lg:items-start">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="space-y-4">
            <div>
              ...
              <h2 className="mt-1.5 text-lg font-semibold tracking-tight text-stone-950">
                Get a calm digest instead of a flood
              </h2>
            </div>
            <p className="text-[0.8125rem] leading-[1.65] text-slate-600">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <div className="space-y-4">
                ...
                {feedback ? (
                  <p className={`rounded-lg border px-3 py-2 text-sm ${feedbackClassName}`}>{feedback.text}</p>
                ) : null}
              </div>
            </div>
          </div>
